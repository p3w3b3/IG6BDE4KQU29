...
{
  document.getElementById("sm1p6button").addEventListener("click", (event) => {
    $(".section1selection").css("background-color", "white");
    $(".section1div").hide();
    $("#sm1p6video").show();
    $("#sm1p6button").css("background-color", "#e7edf0");
  });
}
...
let prognumsm = 0;
    ...
    if (document.querySelector("#sm1p6x").style.display == "block") {
      prognumsm++;
    }
...
}
{
  ...
  var finished = function () {
    ...
    firebase
      .firestore()
      .doc("users/" + firebase.auth().currentUser.uid)
      .set({ pizza3p5: true }, { merge: true });
    checkcompleted1()
  };
  player.on("ended", finished);
}
{
  var iframe = document.querySelector("#sm1p6video");
  var player = new Vimeo.Player(iframe);
  var finished = function () {
    videoended = "true";
    $("#sm1p6").hide();
